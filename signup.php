<?php
    ...
    if($_SERVER["REQUEST_METHOD"]=="POST"){
        require "config.php";
        $sql = $conn->prepare("SELECT COUNT(*) FROM users WHERE phone = ? OR email = ?");
        $sql->bindParam(1, $_POST["phone"], PDO::PARAM_INT);
        ...
        if($count>0){
            ...
            $sql = $conn->prepare("INSERT INTO users (name, phone, email, password, type) VALUES (?, ?, ?, ?, ?)");
            ...
            $sql->bindParam(5, $_POST["type"], PDO::PARAM_BOOL);
            try{
                $sql->execute();
                $sql = $conn->prepare("SELECT id FROM users WHERE phone = ? AND email = ?");
                $sql->bindParam(1, $_POST["phone"], PDO::PARAM_INT);
                $sql->bindParam(2, $_POST["email"], PDO::PARAM_STR);
                try{
                    $sql->execute();
                    session_start();
                    $_SESSION["id"] = $sql->fetch(PDO::FETCH_NUM)[0];
                    header("Location: profile");
            ...
            <div class="container-xl">
                ...
                <div class="collapse navbar-collapse flex-grow-0">
                    <ul class="navbar-nav d-none d-md-flex">
                        <li class="nav-item"><a href="#" class="nav-link fw-bold"><i class="fa-solid fa-shop fa-lg me-2"></i><span>Businesses</span></a></li>
                        <li class="nav-item"><a href="#" class="nav-link fw-bold"><i class="fa-solid fa-user-tie fa-lg me-2"></i><span>Volunteers</span></a></li>
                        <li class="nav-item"><a href="#" class="nav-link fw-bold"><i class="fa-regular fa-circle-question fa-lg me-2"></i><span>Need Help</span></a></li>
                        <li class="nav-item"><a href="login" class="nav-link fw-bold"><i class="fa-regular fa-circle-user fa-lg me-2"></i><span>Login</span></a></li>
                        <li class="nav-item"><a href="signup" class="btn btn-success">Register</a></li>
                    </ul>
                </div>
            </div>
            <a href="login" class="btn border-0 d-md-none"><i class="fa-regular fa-circle-user fa-xl"></i></a>
        </nav>
    </header>
    <main class="flex-grow-1">
        <article class="container-xxl py-3">
            <form action=<?php echo $_SERVER["PHP_SELF"];?> method="post" class="mx-auto my-5" style="max-width: 400px">
                <div class="nav nav-underline nav-fill">
                    <a href="login" class="nav-link">Login</a>
                    <a href="signup" class="nav-link active">Signup</a>
                </div>
                <div class="py-5">
                    <h3 class="text-center">Join VyaparGuru!</h3>
                    <?php
                        if(isset($error)){
                    ...
                    <div class="form-floating">
                        <input type="text" id="name" name="name" class="form-control" placeholder="Full name" autocomplete="off" spellcheck="false" required>
                        <label for="name">Full name</label>
                    </div>
                    <div class="form-floating my-3">
                        <input type="tel" id="phone" name="phone" class="form-control" placeholder="Phone number" autocomplete="off" minlength="10" maxlength="10" required>
                        <label for="phone">Phone number</label>
                    </div>
                    <div class="form-floating">
                        <input type="email" id="email" name="email" class="form-control" placeholder="Email address" autocomplete="off" spellcheck="false" required>
                    ...
                    <div class="my-3">By continuing, you agree to our <a href="#" class="text-decoration-none">Terms</a> and <a href="#" class="text-decoration-none">Privacy</a>.</div>
                    <button type="submit" class="btn btn-primary w-100">Signup</button>
                </div>
            </form>
        </article>
    </main>
    <footer class="nav justify-content-center border-top py-3">
        <a href="#" class="link-dark text-decoration-none my-1 mx-2">About us</a>
        <a href="#" class="link-dark text-decoration-none my-1 mx-2">Our stories</a>
        <a href="#" class="link-dark text-decoration-none my-1 mx-2">Top Volunteers</a>
        <a href="#" class="link-dark text-decoration-none my-1 mx-2">Businesses</a>
    </footer>
</body>
</html>